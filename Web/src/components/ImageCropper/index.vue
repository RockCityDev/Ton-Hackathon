  ...
  <div v-show="value" class="vue-image-crop-upload">
    <div class="vicp-wrap">
      <div class="headers clearfix">
        <div class="txt fl">选择图片</div>
        <i class="el-icon-close fr" @click="off" />
      </div>
      ...
      <div v-if="step == 2" class="vicp-step2">
        <div class="tt"> 裁剪图片</div>
        <div class="vicp-crop">
          <div v-show="true" class="vicp-crop-left">
            <div ref="vicp-img-container" class="vicp-img-container">
              <div ref="img-boxs" class="img-boxs">
                <img
                  ref="img"
                  :src="sourceImgUrl"
              ...
              <div class="vicp-img-shade vicp-img-shade-3" />
              <div class="vicp-img-shade vicp-img-shade-4" />
            </div>
            <div class="vicp-range" style="display: none;">
              <input
                :value="scale.range"
                type="range"
                step="1"
          ...
          <div class="vicp-crop-center" />

          <div v-show="true" class="vicp-crop-right">
            <div v-show="createImgUrl" class="vicp-preview">
              <span class="tt">{{ lang.preview }}</span>
        ...
        <div class="vicp-operate vicp-operate2">
          <el-button type="primary" size="mini" @click="prepareUpload" @mousedown="ripple">确定</el-button>
...
// import { postUpload } from '@/api/http-request'
import { uploadImage } from '@/api/request'
import language from './utils/language.js'
import mimes from './utils/mimes.js'
// import data2blob from './utils/data2blob.js'
import effectRipple from './utils/effectRipple.js'
// import { alertMsg } from '@/utils/init'
...
export default {
  ...
  mounted() {
    // 监听鼠标滚动事件
    const that = this
    window.addEventListener( 'mousewheel', that.debounce( that.handleScroll, 100 ), true ) ||
    window.addEventListener( 'DOMMouseScroll', that.debounce( that.handleScroll, 100 ), false )
  },
  destroyed() {
  ...
  methods : {
    ...
    handleChange( e ) {
      ...
      if ( this.loading !== 1 ) {
        ...
        if ( this.checkFile( files[0] ) ) {
          // this.setSourceImg( files[0] ) TODO
          // 上传图片
          this.uploadImage( files[0] )
        } else {
          // alertMsg( this, '图片大小不超过5M的png和JPEG格式', '格式错误', '确定' )
        }
      }
    },
    // 上传图片
      ...
      uploadImage( fmData )
        ...
        .catch( err => {
          if ( that.value ) {
            ...
            that.$emit( 'crop-upload-fail', err )
          }
        } )
    },
    /* ---------------------------------------------------------------*/
    ...
    startCrop() {
      ...
      const sim = sourceImgMasking
      const img = new Image()
      // // 图片跨域 https://www.cnblogs.com/iroading/p/11011268.html
      // img.setAttribute( 'crossOrigin', 'anonymous' )
      img.src = sourceImgUrl
      img.onload = () => {
        const nWidth = img.naturalWidth
        const nHeight = img.naturalHeight
        this.imgOriginSize.width = nWidth
        this.imgOriginSize.height = nHeight
    ...
    getBoundingClientRects() {
      setTimeout( () => {
        // const imgContainer = this.$refs['vicp-img-container'] // 图片父级的父级,整个大容器的宽高
        const imgBoxs = this.$refs['img-boxs'] // 图片父级，宽高固定
        const imgEl = this.$refs.img // 图片

        const imgBoxsWidth = imgBoxs.offsetWidth // 获取图片父级宽度
        const imgBoxsHeight = imgBoxs.offsetHeight // 获取图片父级高度

        const imgWidth = imgEl.offsetWidth
        const imgHeight = imgEl.offsetHeight
        const imgLeft = imgEl.offsetLeft // 获取图片距离左侧宽度
        const imgTop = imgEl.offsetTop // 获取图片距离顶部高度

        const nWidth = this.imgOriginSize.width
        const nHeight = this.imgOriginSize.height
        const scaleWidth = parseFloat( ( imgWidth / nWidth ).toFixed( 2 ) ) // 获取图片宽度缩放因子
        const scaleHeight = parseFloat( ( imgHeight / nHeight ).toFixed( 2 ) ) // 获取图片宽度缩放因子

        // 获取图片 left top 的相对坐标百分比，好换算成原始图片的
        const leftPercent = parseFloat( ( imgLeft / imgWidth ).toFixed( 2 ) )
        const topPercent = parseFloat( ( imgTop / imgHeight ).toFixed( 2 ) )
        const leftPercentReal = Math.abs( parseInt( nWidth * leftPercent ) )
        const topPercentReal = Math.abs( parseInt( nHeight * topPercent ) )

        // 获取截取的宽高 ： 根据父容器的宽高以及缩放比例计算出新宽高
        const imgCropWidthReal = parseInt( imgBoxsWidth / scaleWidth )
        const imgCropHeightReal = parseInt( imgBoxsHeight / scaleHeight )

        // 拼接出公有云图片裁剪参数
        const paramsStr = `?x-image-process=image/crop,x_${leftPercentReal},y_${topPercentReal},w_${imgCropWidthReal},h_${imgCropHeightReal}`
        this.createImgUrl = this.sourceImgUrl + paramsStr
      }, 200 )
    },
    // 鼠标按下图片准备移动
    imgStartMove( e ) {
...
<style lang="scss">
  ...
  .vue-image-crop-upload .vicp-wrap .vicp-step2 .vicp-crop .vicp-crop-center {
    float: left;
    margin: 0 50px;
    width: 1px;
  ...
  .vicp-crop-left
  .vicp-img-container
  .vicp-img-shade.vicp-img-shade-3 {
    top: 0;
    left: 30px;
    height: 70px;
  ...
  .vicp-crop-left
  .vicp-img-container
  .vicp-img-shade.vicp-img-shade-4 {
    bottom: 0;
    left: 30px;
    height: 70px;
    width: calc(262px - 60px);
